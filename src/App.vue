  ...
  <a-layout style="min-height: 100vh">
    <a-layout-sider v-if="isLoggedIn" v-model:collapsed="collapsed" collapsible>
      ...
      <a-menu v-model:selectedKeys="selectedKeys" theme="dark" mode="inline" @click="handleMenuClick">
        ...
        <a-menu-item key="permissions" v-if="userRole === 'SUPER' || userRole === 'ADMIN'">
          <setting-outlined />
          <span>权限管理</span>
        </a-menu-item>
...
watch(route, (newRoute) => {
    ...
    else if (newRoute.name === 'ProjectList') selectedKeys.value = ['projects'];
    else if (newRoute.name === 'PermissionManagement') selectedKeys.value = ['permissions'];
}, { immediate: true });
